...
public abstract class IEnumerable<TSource> implements Iterable<TSource>
{
	 ...
	 * @return A Enumerable that contains a range of sequential integral numbers.
	 */
	...
	/**
	 * Returns the input typed as a ParallelEnumerable(Of T)
	 *
	 * @return Parallel Enumeration of this IEnumerable(Of T)
	 */
	public ParallelEnumerable<TSource> AsParallel()
	{
		return new ParallelEnumerable<>(this);
	}
	 ...
	 *
	 * @param selector
	 *            An accumulator function to be invoked on each element (TSouce-Old, TSource-Current),
	 *            output TSource-New.
	 ...
	 * @return The final accumulator value, if any parameters are null, returns seed
	 *         if any parameters are not set, returns null
	 */
	public <TAccumulate> TAccumulate Aggregate(TAccumulate seed, BiFunction<TAccumulate, TSource, TAccumulate> selector)
	{
		// parameter check
	 ...
	 *         if any parameters are not set, returns null
	 */
	public <TResult, TAccumulate> TResult Aggregate(TAccumulate seed, BiFunction<TAccumulate, TSource, TAccumulate> selector, Function<TAccumulate, TResult> resultSelector)
	{
		// parameter check
	 ...
	 * @return A Enumerable that contains the concatenated elements of the two input sequences.
	 */
	 ...
	 * @return A Enumerable that contains distinct elements from the source sequence.
	 */
	 ...
	 * @return null if the index is outside the bounds of the source sequence; otherwise, the element at the specified
	 ...
	 * @param second
	 *            A Enumerable(Of T) whose elements that also occur in the first sequence will cause those elements to
	 *            be
	 ...
	 */
	public <TResult extends Comparable<TResult>> IEnumerable<TSource> Except(IEnumerable<TSource> second, Function<TSource, TResult> selector)
	{
		return Except(second, (first, next) -> selector.apply(first).compareTo(selector.apply(next)));
	}

	/**
	 * Produces the set difference of two sequences by using the specified Comparator(Of T) to compare values.
	 *
	 * @param second
	 *            A Enumerable(Of T) whose elements that also occur in the first sequence will cause those elements to
	 *            be
	 ...
	 * @param p
	 *            A function to test each element for a condition.
	 * @return default(TSource) if source is empty or if no element passes the test specified by predicate; otherwise,
	 ...
	 *
	 * @param action
	 *            The Action delegate to perform on each element of the Enumerable.
	 */
	 ...
	 * @return A Enumerable(Of Grouping(Of TKey, TSource)) where each Grouping(Of TKey, TElement) object contains a
	 *         sequence
	 *         of objects and a key.
	 *         if any parameters are null, return empty set
	 */
	public <TKey extends Comparable<TKey>> IEnumerable<Grouping<TKey, TSource>> GroupBy(Function<TSource, TKey> keySelector)
	{
	 ...
	 */
	public <TKey> IEnumerable<Grouping<TKey, TSource>> GroupBy(Function<TSource, TKey> keySelector, Comparator<TKey> comparer)
	{
	 ...
	 *            A function to map each source element to an element in the Grouping(Of TKey, TElement).
	 * @return An Enumerable(Of IGrouping(Of TKey, TElement)) where each Grouping(Of TKey, TElement) object contains a
	 *         collection of objects of type TElement and a key.
	 */
	public <TKey extends Comparable<TKey>, TElement> IEnumerable<Grouping<TKey, TElement>> GroupBy(Function<TSource, TKey> keySelector, Function<TSource, TElement> elementSelector)
	{
		// return GroupBy(keySelector, elementSelector, (first, second) -> first.compareTo(second));
		return GroupBy(keySelector, (key, EnumerableOfTSource) -> new Grouping<>(key, EnumerableOfTSource.Select(ts -> elementSelector.apply(ts)).ToList()),
				(first, second) -> first.compareTo(second)); // this line doesn't cause java to detect an ambiguous call
	}
	 ...
	 * @return A Enumerable(Of Grouping(Of TKey, TElement)) where each Grouping(Of TKey, TElement) object contains a
	 *         collection of objects of type TElement and a key.
	 *         if any parameters are not set, returns empty set
	 */
	public <TKey, TElement> IEnumerable<Grouping<TKey, TElement>> GroupBy(Function<TSource, TKey> keySelector, Function<TSource, TElement> elementSelector,
			Comparator<TKey> comparer)
	{
		return GroupBy(keySelector, (key, EnumerableOfTSource) -> new Grouping<>(key, EnumerableOfTSource.Select(ts -> elementSelector.apply(ts)).ToList()), comparer);
	}
	 ...
	 * @return A Enumerable(Of TResult) where each element represents a projection over a group and its key.
	 *         if any parameters are not set, returns empty set
	 */
	 ...
	 *         if any parameters are not set, returns empty set
	 */
	public <TKey, TResult> IEnumerable<TResult> GroupBy(Function<TSource, TKey> keySelector, BiFunction<TKey, IEnumerable<TSource>, TResult> resultSelector,
			Comparator<TKey> comparer)
	{
	 ...
	 *            A function to extract the key for each element.
	 * @param elementSelector
	 *            A function to map each source element to an element in a Grouping(Of TKey, TElement).
	 * @param resultSelector
	 *            A function to create a result value from each group.
	 * @return A Enumerable(Of TResult) where each element represents a projection over a group and its key.
	 *         if any parameters are not set, returns an empty set
	 */
	public <TKey extends Comparable<TKey>, TElement, TResult> IEnumerable<TResult> GroupBy(Function<TSource, TKey> keySelector, Function<TSource, TElement> elementSelector,
	...
	/**
	 * Groups the elements of a sequence according to a specified key selector function and creates a result value from
	 * each group and its key. Key values are compared by using a specified comparer, and the elements of each group are
	 ...
	 *         if any parameters are null, return empty set
	 */
	public <TKey, TElement, TResult> IEnumerable<TResult> GroupBy(Function<TSource, TKey> keySelector, Function<TSource, TElement> elementSelector,
			BiFunction<TKey, IEnumerable<TElement>, TResult> resultSelector, Comparator<TKey> comparer)
	{
		...
		IEnumerable<TupleT2<Integer, TSource>> orderedSourceElements = this.Select((src, i) -> new TupleT2<>(i, src)).OrderBy(t -> keySelector.apply(t.Item2), comparer);
	 ...
	 * (LEFT JOIN inner ON innerKeySelector == outerKeySelector) using the default comparer.
	 *
	 * @param <TInner>
	 ...
	 * @param resultSelector
	 *            A function to create a result element from an element from the first sequence and a collection of
	 *            matching elements from the second sequence.
	 * @return A Enumerable(Of T) that contains elements of type TResult that are obtained by performing a grouped join
	 *         on
	 *         two sequences.
	 */
	public <TInner, TKey extends Comparable<TKey>, TResult> IEnumerable<TResult> GroupJoin(IEnumerable<TInner> inner, Function<TSource, TKey> outerKeySelector,
			Function<TInner, TKey> innerKeySelector, BiFunction<TSource, IEnumerable<TInner>, TResult> resultSelector)
	{
	...
	/**
	 * Correlates the elements of two sequences based on equality of keys and groups the results.
	 * (LEFT JOIN inner ON innerKeySelector == outerKeySelector) using the specified comparer.
	 *
	 * @param <TInner>
	 *            The type of the elements of the second sequence.
	 * @param <TKey>
	 ...
	 *         two sequences.
	 */
	public <TInner, TKey, TResult> IEnumerable<TResult> GroupJoin(IEnumerable<TInner> inner, Function<TSource, TKey> outerKeySelector, Function<TInner, TKey> innerKeySelector,
			BiFunction<TSource, IEnumerable<TInner>, TResult> resultSelector, Comparator<TKey> comparer)
	{
		...
		IEnumerable<TResult> result = this.Select(s -> resultSelector.apply(s, innerGroup.Item(outerKeySelector.apply(s), comparer)));
		return result;
	}
	 ...
	 *            The type of the resulting value.
	 * @param second
	 *            A Enumerable whose distinct elements that also appear in the first sequence will be returned.
	 * @param selector
	 ...
	 */
	public <TResult extends Comparable<TResult>> IEnumerable<TSource> Intersect(IEnumerable<TSource> second, Function<TSource, TResult> selector)
	{
	...
	/**
	 * Determines whether a sequence contains a specified element by using a specified Comparator
	 *
	 * @param value
	 *            The value to locate in the sequence.
	 * @param comparer
	 ...
	 * @param p
	 *            A function to test each element for a condition.
	 * @return null if the sequence is empty or if no elements pass the test in the predicate function; otherwise, the
	 ...
	 */
	public <TResult extends Comparable<TResult>> OrderedEnumerable<TSource> OrderBy(Function<TSource, TResult> keySelector)
	{
	 ...
	 * @return ordered collection, if any parameters are null it returns the current collection
	 */
	public <TResult> OrderedEnumerable<TSource> OrderBy(Function<TSource, TResult> selector, Comparator<TResult> comparer)
	{
	 ...
	 * @param selector
	 *            A function to extract a key from an element.
	 * @return A Enumerable whose elements are sorted in descending order according to a key.
	 */
	public <TResult extends Comparable<TResult>> OrderedEnumerable<TSource> OrderByDescending(Function<TSource, TResult> selector)
	{
		// OrderBy + O(n/2)
	 ...
	 * @return A Enumerable whose elements are sorted in descending order according to a key.
	 */
	public <TResult> OrderedEnumerable<TSource> OrderByDescending(Function<TSource, TResult> selector, Comparator<TResult> comparer)
	{
	 ...
	 * @return A Enumerable whose elements are the result of invoking the transform function on each element of
	 *         source.
	 */
	 ...
	 * @param selector
	 *            A transform function to apply to each element.
	 * @return A Enumerable whose elements are the result of invoking the one-to-many transform function on each
	 *         element
	 ...
	 * @return A Enumerable whose elements are the result of invoking the one-to-many transform function
	 ...
	 */
	public <TResult, TCollection> IEnumerable<TResult> SelectMany(Function<TSource, IEnumerable<TCollection>> collectionSelector,
	 ...
	 *         corresponding source element to a result element.
	 */
	public <TResult, TCollection> IEnumerable<TResult> SelectMany(BiFunction<TSource, Integer, IEnumerable<TCollection>> collectionSelector,
			BiFunction<TSource, TCollection, TResult> resultSelector)
	{
		// default
		IEnumerable<TResult> result = new List<>();
